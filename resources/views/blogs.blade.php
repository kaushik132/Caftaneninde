    ...
    <section class="lg:px-12 px-4 lg:py-10 py-8">
        <div class="grid lg:grid-cols-2 items-center lg:gap-8 gap-4 border-[#D0D0D0] border lg:rounded-2xl rounded-md">
            <div class="col-span-1 h-[250px] lg:h-[400px] relative">
                <img class="h-full w-full object-cover rounded-t-md lg:rounded-l-2xl" src="./images/video.jpg"
                    alt="">
                <span
                    class="text-white text-[12px] bg-[#FF71A8] rounded-md px-4 py-1.5 inline-block absolute top-4 left-5">Featured</span>
            </div>

            <div class="col-span-1  px-4">
                <span
                    class=" text-[10px] border-[#9D9D9D] border rounded-md lg:rounded-xl px-4 font-medium py-1 inline-block">Bride
                    Tips</span>

                <h2 class="mt-2 lg:text-[32px] text-[20px] lg:pr-32 font-medium lg:leading-[44px]">The Ultimate Guide to
                    Choosing Your Wedding Gown</h2>

                <p class="lg:text-[14px] text-[12px] font-medium mt-1">Discover expert tips and advice for finding the
                    perfect wedding dress that makes you feel beautiful on your special day.</p>
                ...
                <a href="{{ url('blog-details') }}"
                    class="group/btn bg-[#FF71A8] hover:bg-black mb-4 lg:mb-0 mt-5 px-6 lg:py-2.5 py-2 rounded-md inline-flex items-center gap-2 text-[13px] lg:text-[15px] text-white font-medium transition-all duration-300">
                    Read More
                    <i
                        class="fa-solid fa-arrow-right lg:text-[12px] text-[10px] transition-transform duration-300 group-hover/btn:translate-x-1.5"></i>
                </a>
            </div>
        </div>
    </section>
    ...
    <section class="lg:px-10 px-4">
        ...
        <div class="grid lg:grid-cols-3 grid-cols-1 lg:gap-8 gap-3 mt-3">
            ...
            <div class="col-span-1 border-[#CCCCCC] border rounded-md lg:rounded-2xl">
                <img class="lg:h-[260px] h-[180px] object-cover rounded-t-md lg:rounded-t-2xl object-center  w-full"
                    src="{{url('uploads/'. $blog->thumbnail)}}" alt="{{ $blog->title}}">

                <div class="lg:px-5 px-3 py-3">
                    <p
                        class="lg:text-[12px] text-[10px] font-medium inline-block px-4 py-1 border-[#DBDBDB] border rounded-lg">
                       {{$blog->category->name}}</p>

                    <h2 class="lg:text-[17px] text-[13px] mt-3 font-medium">{{$blog->title}}</h2>

                    <p class="text-[#686868] mt-1 text-[11px] lg:text-[13px] font-medium">{!! Str::limit($blog->content, 100) !!}</p>
                    ...
                    <div class="mt-3 lg:flex gap-6">
                        <div class="text-[#525252] flex lg:gap-2 gap-1 items-center  lg:items-start">
                            ...
                            <p class="lg:text-[13px] text-[12px]">{{ $blog->publish_date->format('F d, Y') }}</p>
                        </div>

                        <div class="text-[#525252] flex lg:gap-2 gap-1 items-start">
                            <i class="fa-regular fa-clock lg:text-[14px] mt-0.5 text-[12px]"></i>
                            <p class="lg:text-[13px] text-[12px]">{{$blog->read_time}} min read</p>
                        </div>
                    </div>

                    <a href="{{ url('blog-details/'. $blog->slug) }}"
                        class="group/read lg:text-[14px] text-[12px] inline-flex items-center lg:mt-4 mt-3 font-bold text-[#FF71A8] hover:text-black transition-colors duration-300">
                        <span class="border-b border-transparent group-hover/read:border-black transition-all">Read
                            More</span>
                        <i
                            class="fa-solid fa-arrow-right text-[13px] ml-1 transition-transform duration-300 group-hover/read:translate-x-1.5"></i>
                    </a>
                </div>
            </div>
                        @endforeach
