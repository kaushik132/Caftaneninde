...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (mainImg && zoomArea) {
        ...
        thumbs.forEach((img, index) => {
            img.addEventListener("click", () => {
                ...
                thumbBoxes[index].classList.replace(
                    "border-transparent",
                    "border-[#FF71A8]",
                );
    ...
    colorDots.forEach((dot) => {
        dot.onclick = function () {
            colorDots.forEach((d) =>
                d.classList.remove("ring-2", "ring-[#FF71A8]"),
            );
...
function showToast(msg) {
    ...
    toast.className =
        "custom-toast fixed bottom-5 right-5 bg-black text-white px-6 py-3 rounded-md shadow-2xl z-[9999] animate-slideUp";
...
function switchTab(t) {
    ...
    document
        .querySelectorAll(`.sidebar-link[data-tab="${t}"]`)
        .forEach((l) => l.classList.add("active"));
    document
        .querySelectorAll(".mobile-tab-item")
        .forEach((i) => i.classList.remove("active"));
    document
        .querySelectorAll(`.mobile-tab-item[data-tab="${t}"]`)
...
document.querySelectorAll(".mobile-tab-item[data-tab]").forEach((i) =>
...
document.querySelectorAll("[data-order-tab]").forEach((btn) => {
    btn.addEventListener("click", function () {
        ...
        document.querySelectorAll(".order-card").forEach((c) => {
            c.style.display =
                f === "all" || c.dataset.status === f ? "block" : "none";
        });
    });
});
...
function openAddressModal(id) {
    ...
    if (id && addressData[id]) {
        ...
        const radio = document.querySelector(
            `input[name="addr-type"][value="${a.type}"]`,
        );
        ...
        document.querySelector(
            'input[name="addr-type"][value="Home"]',
        ).checked = true;
        [
            "addr-name",
            "addr-phone",
...
function saveAddress() {
    ...
    if (isDefault)
        Object.values(addressData).forEach((a) => (a.isDefault = false));

    if (id && addressData[id]) {
        addressData[id] = { type, name, phone, street, city, pin, isDefault };
        showToast("✅ Address updated successfully!");
    } else {
        addressData[nextAddrId++] = {
            type,
            name,
            phone,
            street,
...
function updatePassword() {
    ...
    if (nw.length < 8) {
        showToast("⚠️ New password must be 8+ characters");
        return;
    }
    if (nw !== cf) {
...
function clearPassFields() {
    ["cur-pass", "new-pass", "conf-pass"].forEach(
